...
function addMappingDiv(fieldId) {
    item_count++;
...
function getFieldList(event) {
    ...
    if(collection_check.isValid == true) {
        let project_type_id = $(event.target).parent().parent().find("select.fv_project_type_name").val();
...
$("body").on("change", "select[name='is_public_form']", async function () {
    ...
    if (is_public_form == 1) {
        ...
        if (create_fv_project) {
            $(".project-type-div").removeClass("d-none");
        }
    } else {
        $(".create-project-checkbox-div").addClass("d-none");
        $(".project-type-div").addClass("d-none");
    }
});

$("body").on("click", "input[name='create_fv_project']", async function () {
    let create_fv_project = $(this).is(":checked");
